...
Original file is located at
    https://colab.research.google.com/drive/1ebof8dbVWxztkNnN-i_T0GhgdeeNZPdn

# Baseline Model

This model only uses captured screenshots + text to detect if the video is deep faked.
...
for name in presidents:
  for i in range(1,16):
    ...
    sample = {'text': text_file, 'images': image_files, 'text_path': text_path, 'image_path': screenshot_folder_path, 'label': 0}
    data.append(sample)
...
for name in presidents:
  for i in range(1,9):
    ...
    image_files = os.listdir(screenshot_folder_path)
    sample = {'text': text_file, 'images': image_files, 'text_path': text_path, 'image_path': screenshot_folder_path, 'label': 1}
    data.append(sample)
...
label_count = data_df["label"].value_counts()
label_count
...
train, test = unpack_dataframe(train_df), unpack_dataframe(test_df)
...
"""There is a significant imbalance between real and fake data. There are several ways to tackle this. We can:

1.   Augment the Fake Data
2.   Reduce the number of real data
